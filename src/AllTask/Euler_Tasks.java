package AllTask;

import java.util.ArrayList;
import java.util.List;

/**
 * Created by valeriyartemenko on 13.08.17.
 */
public class Euler_Tasks {
    public static int sumSquareDifference() {
        int sumSquare = 0;
        int totalSumNatural = 0;
        int squareSum = 0;
        for (int i = 1; i < 101; i++) {
            totalSumNatural += i;
            sumSquare += Math.pow(i, 2);
        }
        squareSum = (int) Math.pow(totalSumNatural, 2);
        return squareSum - sumSquare;
    }

    public static long primeNumber() {
        long divisor = 0; //количество делителей
        List<Long> primeList = new ArrayList<>();
        primeList.add((long) 1);
        label:
        for (long i = 0; primeList.size() <= 10001; i++) {
            for (long j = 1; j <= i; j++) {      // перебираем делители в диапазоне
                if (i % j == 0) { // проверяем  делимость числа на цело
                    divisor++; // считаем делители

                }
                if (divisor > 2) {    // данная проверка для оптимизации,
                    divisor = 0;    // ускорила работу на том же интервале
                    continue label;    // в 4.91 раза
                }

            }
            if (divisor == 2) {    //  делителей два
                primeList.add(i);
            }
            divisor = 0;
        }
        return primeList.get(primeList.size() - 1);
    }

    public static void largestProductInASeries() {
        List<Long> numbersList = new ArrayList<>();
        String numbers = "73167176531330624919225119674426574742355349194934" +
                "96983520312774506326239578318016984801869478851843" +
                "85861560789112949495459501737958331952853208805511" +
                "12540698747158523863050715693290963295227443043557" +
                "66896648950445244523161731856403098711121722383113" +
                "62229893423380308135336276614282806444486645238749" +
                "30358907296290491560440772390713810515859307960866" +
                "70172427121883998797908792274921901699720888093776" +
                "65727333001053367881220235421809751254540594752243" +
                "52584907711670556013604839586446706324415722155397" +
                "53697817977846174064955149290862569321978468622482" +
                "83972241375657056057490261407972968652414535100474" +
                "82166370484403199890008895243450658541227588666881" +
                "16427171479924442928230863465674813919123162824586" +
                "17866458359124566529476545682848912883142607690042" +
                "24219022671055626321111109370544217506941658960408" +
                "07198403850962455444362981230987879927244284909188" +
                "84580156166097919133875499200524063689912560717606" +
                "05886116467109405077541002256983155200055935729725" +
                "71636269561882670428252483600823257530420752963450";
        long greatestProduct = 0;
        for (int i = 0; i < numbers.length() - 12; i++) {
            long sum = (long) Character.getNumericValue(numbers.charAt(i)) *
                    Character.getNumericValue(numbers.charAt(i + 1)) *
                    Character.getNumericValue(numbers.charAt(i + 2)) *
                    Character.getNumericValue(numbers.charAt(i + 3)) *
                    Character.getNumericValue(numbers.charAt(i + 4)) *
                    Character.getNumericValue(numbers.charAt(i + 5)) *
                    Character.getNumericValue(numbers.charAt(i + 6)) *
                    Character.getNumericValue(numbers.charAt(i + 7)) *
                    Character.getNumericValue(numbers.charAt(i + 8)) *
                    Character.getNumericValue(numbers.charAt(i + 9)) *
                    Character.getNumericValue(numbers.charAt(i + 10)) *
                    Character.getNumericValue(numbers.charAt(i + 11)) *
                    Character.getNumericValue(numbers.charAt(i + 12));
            if (sum > greatestProduct)
                greatestProduct = sum;
        }
        System.out.println(greatestProduct);


    }

    public static void main(String[] args) {
//        System.out.println(sumSquareDifference());
//        System.out.println(primeNumber());
//        largestProductInASeries();
//        System.out.println(Special_Pythagorean_Triplet.genegerateNumbers());
//        System.out.println(200*375*425);
        FindSumPrime.findPrime();
    }
}
